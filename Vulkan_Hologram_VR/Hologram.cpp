...
void Hologram::update_camera() {
    ...
    const glm::mat4 view = glm::lookAt(camera_.eye_pos, camera_.look_pos, camera_.up_vec);
...
void Hologram::on_key(Key key) {
    switch (key) {
        ...
        case KEY_ESC:
            ...
            break;
        /*case KEY_W:
            camera_.eye_pos += glm::vec3(-.1f,.0f,0.0f);
			camera_.look_pos += glm::vec3(-.1f, .0f, 0.0f);
            update_camera();
            break;
        case KEY_S:
            camera_.eye_pos -= glm::vec3(-.1f,.0f,.0f);
			camera_.look_pos -= glm::vec3(-.1f, .0f,.0f);
            update_camera();
            break;
		case KEY_A:
			camera_.eye_pos += glm::vec3(.0f,.1f,0.0f);
			camera_.look_pos += glm::vec3(.0f, .1f, 0.0f);
			update_camera();
			break;
		case KEY_D:
			camera_.eye_pos -= glm::vec3(.0f, .1f, 0.0f);
...
void Hologram::on_mouse_wheel(short wheelDelta) 
{
	float zoomspeed = 0.005*wheelDelta;
	glm::vec3 forward_vec = camera_.eye_pos - camera_.look_pos;

	camera_.eye_pos += zoomspeed * forward_vec;
	camera_.look_pos += zoomspeed*forward_vec;
	update_camera();
	return;
}
glm::mat3 Hologram::get_RotM(float theta,float X, float Y, float Z)
{
// get unit vector of (X,Y,Z)
	const float vec_size = sqrt(X*X + Y * Y + Z * Z);
	if (vec_size == 0) return glm::mat3(0.0f);
	const float x = X/ vec_size;
	const float y = Y / vec_size;
	const float z = Z / vec_size;
	float const cos_theta = cos(theta);
	float const sin_theta = sin(theta);
	float const cos_1_theta = 1 - cos_theta;

	glm::mat3 RotM	{ cos_theta + x * x *cos_1_theta,		    x*y*cos_1_theta - z * sin_theta,		x*z*cos_1_theta + y * sin_theta,
													y*x*cos_1_theta + z * sin_theta,		cos_theta + y * y*cos_1_theta,				y*z*cos_1_theta - x * sin_theta,
													z*x*cos_1_theta - y * sin_theta,			z*y*cos_1_theta + x * sin_theta,		cos_theta + z * z*cos_1_theta		};
	return RotM;
}
void Hologram::on_mouse_move(int32_t x, int32_t y)
{
	int32_t dx = (int32_t)mousePos_.x - x;
	...
	if (mouseButtons_.left) {
		...
		//
		eye_pos   = rotMy * rotMx * eye_pos;
		look_pos = rotMy * rotMx * look_pos;
		up_vec = rotMy * rotMx*up_vec;
		//
		camera_.eye_pos = eye_pos + center_vec;
		camera_.look_pos = look_pos + center_vec;
