...
LRESULT ShellWin32::handle_message(HWND hWnd, UINT msg, WPARAM wparam, LPARAM lparam) {
    switch (msg) {
		...
		case WM_LBUTTONDOWN: 
		{
			game_.mousePos_.x = (float)LOWORD(lparam);
			game_.mousePos_.y = (float)HIWORD(lparam);
			game_.mouseButtons_.left = true; 
		}
			break;
		case WM_RBUTTONDOWN:
		{
			game_.mousePos_.x = (float)LOWORD(lparam);
			game_.mousePos_.y = (float)HIWORD(lparam);
			game_.mouseButtons_.right = true;
		}
			break;
		case WM_MBUTTONDOWN:
		{
			game_.mousePos_.x = (float)LOWORD(lparam);
			game_.mousePos_.y = (float)HIWORD(lparam);
			game_.mouseButtons_.middle = true;
		}
			break;

		case WM_LBUTTONUP:
			game_.mouseButtons_.left = false;
			break;
		case WM_RBUTTONUP:
			game_.mouseButtons_.right = false;
			break;
		case WM_MBUTTONUP:
			game_.mouseButtons_.middle = false;
			break;
		case WM_MOUSEWHEEL:
		{
			short wheelDelta = (short)HIWORD(wparam);
			game_.on_mouse_wheel(wheelDelta);
			break;
		}
		case WM_MOUSEMOVE:
		{
			game_.on_mouse_move((int32_t) LOWORD(lparam), (int32_t) HIWORD(lparam));
			break;
		}
